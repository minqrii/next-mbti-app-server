...
const getMessageCount = async function (data) {
    try {
        const messageCount = await whisperAppServer.get(`/v1/messages/${data.address}/count?networkId=${data.networkId}&contractAddressQuery=${data.contractAddressQuery}&serviceName=${data.serviceName}`);
...
const getMessagesByAddress = async function (data) {
    try {
        const messageData = await whisperAppServer.get(`/v1/messages/${data.toAddress}/address/${data.fromAddress}?networkId=${data.networkId}&contractAddressQuery=${data.contractAddressQuery}&serviceName=${data.serviceName}`);
