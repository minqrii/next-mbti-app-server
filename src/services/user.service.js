...
const getSpamUsers = async function (data) {
    try {
        const spamUsers = await whisperAppServer.get(`/v1/users/${data.address}/spam?networkId=${data.networkId}&contractAddress=${data.contractAddress}&serviceName=${data.serviceName}`);
